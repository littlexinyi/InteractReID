...
def load_checkpoint(model, config):
    if config.model.ckpt_type == 'pretrain':
    ...
    elif config.model.ckpt_type == 'finetune_train':
        if(config.experiment.patch_size == 16):
            ckpt = torch.load(os.path.join(config.model.saved_path, 'simplified_trained_ViT-B-16.pth'), map_location='cpu')
        elif(config.experiment.patch_size == 32):
            ckpt = torch.load(os.path.join(config.model.saved_path, 'simplified_trained_ViT-B-32.pth'), map_location='cpu')
        new_state = ckpt['model']
    elif config.model.ckpt_type == 'direct_test':
        if(config.data.dataset == 'Tri-PEDES'): 
            ...
            ckpt = torch.load(os.path.join(config.model.output_path, 'checkpoint_best.pth'), map_location='cpu')
...
def load_checkpoint_vit(vision, config):
    # ckpt = torch.load(os.path.join(config.model.saved_path, 'simplified_trained_ViT-B-16.pth'), map_location='cpu')
    ...
    for name, params in state.items():
        ...
        if(name.startswith('sketch_encoder')):
            new_name = name.replace('sketch_encoder.', '')
            new_state[new_name] = params
    # for name, p in new_state.items():
    #     print("load_vision_param:", name)
    # for name, p in vision.named_parameters():
    #     print("sketch_encoder:", name)
    load_result = vision.load_state_dict(new_state, strict=False)
    return vision, load_result
...
def build_optimizer(config, model):
    ...
    schedule_config = config.schedule
    for n, p in model.named_parameters():
        if not p.requires_grad:     #只添加没有被frozen的参数
            continue  # frozen weights
        ...
        if "mapping" in n:
            ratio = ratio * schedule_config.ratio_factor  # default 10.0

        params += [{"params": [p], "weight_decay": weight_decay, "ratio": ratio}]

    optimizer = torch.optim.AdamW(params, lr=schedule_config.lr, betas=schedule_config.betas, eps=schedule_config.eps, weight_decay=schedule_config.weight_decay)
